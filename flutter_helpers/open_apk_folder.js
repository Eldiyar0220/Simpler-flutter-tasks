...
async function buildApkAndOpenFolder() {
	...
	const workspaceFolder = workspaceFolders && workspaceFolders.length > 0 ? workspaceFolders[0].uri.fsPath : undefined;
	if (workspaceFolder) {
		let apkPath = path.join(workspaceFolder, 'build', 'app', 'outputs', 'flutter-apk', 'app-release.apk');
		if (fs.existsSync(apkPath)) {
			let openCommand;
			if (process.platform === 'darwin') {
				openCommand = 'open';
				apkPath = path.join(workspaceFolder, 'build', 'app', 'outputs', 'flutter-apk', 'app-release.apk');
